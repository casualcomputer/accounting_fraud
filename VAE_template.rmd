...
```{r train_test, echo=TRUE}
    ...
    # for each company, get the earliest and latest year for which a record was available. 
        # for a company, if records are missing in-between years, code missing records with "1111"
            ...
            # helper function: given a gvkey and a year, output a row
              append_missing_record <- function(fyear, gvkey,new_coding, dataset,rowNames_dataset=names(dataset)){ #note that new_coding has to be a text. handle exceptions..
                  output_row <- data.frame(matrix(new_coding,nrow=1,ncol=num.of.cols))
                  names(output_row) <- rowNames_dataset
                  dataset <- rbind(dataset,output_row)
              }
        
            # fill rows for missed filings
              gvkey_with_missing_val <- c(companies_with_missing_year$gvkey)
              for (company in gvkey_with_missing_val){
                  cat("processing gvkey:")
                  company_data <- dataset.new[dataset.new$gvkey==company,]
                  available_years <- company_data$fyear
                  min_fyear <- min(available_years)
                  max_fyear <- max(available_years)
                  missing_years <- setdiff(min_fyear:max_fyear, available_years)
                  for (year in missing_years){
                      append_missing_record(fyear=year,gvkey=company,new_coding="1111",dataset=dataset.new)
                  }
              }
