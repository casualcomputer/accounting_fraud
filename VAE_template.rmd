    ...
    # for each company, get the earliest and latest year for which a record was available. 
        # for a company, if records are missing in-between years, code missing records with "1111"
                   ...
                   #define "pre", "post" and "during" NA's 
                       dataset.new.mod[dataset.new.mod$ind_missing=="pre",3:49] <- -100000000000
                       dataset.new.mod[dataset.new.mod$ind_missing=="post",3:49] <- 100000000000 #-666666666666
                       dataset.new.mod[dataset.new.mod$ind_missing=="during",3:49] <- 100000000000
