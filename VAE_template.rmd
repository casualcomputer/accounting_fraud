...
```{r train_test, echo=TRUE}
    ...
    # for each company, get the earliest and latest year for which a record was available. 
        # for a company, if records are missing in-between years, code missing records with "1111"
                   ...
                   dataset.new.mod<-  dataset.new.mod %>% 
                    ...
                    missing_index <- is.na(dataset.new.mod$sich) 
                    dataset.new.mod.missing <- dataset.new.mod[missing_index,]
                    dataset.new.mod.missing <- as.data.frame(dataset.new.mod.missing)
                    
                    all_rows_missing <- nrow(dataset.new.mod.missing)
                    cat("rows with missing values:", all_rows_missing)
 
                    
                    for (curr_row in 1:nrow(dataset.new.mod.missing)){
                        print(paste("percentage done:",curr_row))
                        if (dataset.new.mod.missing[curr_row,2]<dataset.new.mod.missing[curr_row,51]) {
                            dataset.new.mod.missing[curr_row,3:49] <-rep("pre",47) #before the first filing
                        }

                        if (dataset.new.mod.missing[curr_row,2]>dataset.new.mod.missing[curr_row,50]) {
                            dataset.new.mod.missing[curr_row,3:49] <-rep("missed",47) #in between first and last filing
                        } else {
                            dataset.new.mod.missing[curr_row,3:49] <-rep("post",47) #after the last filing
                        }

                    }
                    
                    dataset.new.mod[missing_index,] <- dataset.new.mod.missing
                    
      #matrix("pre-filing",nrow=pre_IPO_numRows,ncol=47)              
      
              # fill in records before the first filings
                  
    # split into a training set and a test set
        
    # one-hot encoding/convert to the right data type
        dataset.new[sapply(dataset.new,is.integer)] <- lapply(dataset.new[sapply(dataset.new,is.integer)],as.factor)
```
